...
function [ListofSol, Created_Nodes]=RRT_Coll(Agents, myIndex, Obs, MapSize, PathDistance,numerofIt, Created_Nodes, currentK)


start = Agents(myIndex).currentPosition;
goal =  Agents(myIndex).goal;
extParameter = Agents(myIndex).extParameter;
...
while ~nearGoal
    ...
    if(size(listOfPoints,2)~=0)
        for i=1:size(listOfPoints,2)
            ...
            if (collFree)
                ...
                virualK = ceil( (norm(xyz - listOfPoints(i).coordinates)) / extParameter );
                collAgent = collWithOtherAgents(xyz, listOfPoints(i), Agents, myIndex, virualK );
                if(collAgent)
        ...
        if minId
            ...
            for i=1:size(listOfPoints,2)
                ...
                if (collFree)
                    virualK = ceil( (norm(xyz - listOfPoints(i).coordinates)) / extParameter);
                    collAgent = collWithOtherAgents(listOfPoints(i).coordinates, Created_Nodes(numberOfPoints), Agents, myIndex, virualK);
                    if(collAgent)
        ...
        if (collFree)
            virualK = ceil( norm(xyz-closest_vert.coordinates) / extParameter);
            collAgent = collWithOtherAgents(xyz, closest_vert, Agents, myIndex, virualK );
            if(collAgent)
                numberOfPoints=numberOfPoints+1;
                Created_Nodes(numberOfPoints).coordinates = xyz;
    ...
    %     if(mod(numberOfPoints,5)==0)
    %         plotEverithing(rrt_Structure,xy_start,xy_goal,colors);
    %     end
